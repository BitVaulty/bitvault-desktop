...
fn test_vault_creation_view_only_state_flow() {
    ...
    assert_eq!(state.current_step, VaultCreationStep::ScanDescriptorViewOnly);
...
fn test_vault_creation_restore_state_flow() {
    ...
    assert_eq!(state.step_history.contains(&VaultCreationStep::EnterSeedPhrase), true);
    assert_eq!(state.step_history.contains(&VaultCreationStep::ScanDescriptorRestore), true);
}
...
fn test_vault_creation_next_step_for_role() {
    ...
    assert_eq!(state.next_step_for_role(), Some(VaultCreationStep::NameVault));
    
    state.current_step = VaultCreationStep::NameVault;
    assert_eq!(state.next_step_for_role(), Some(VaultCreationStep::SetTimeDelay));
    
    state.current_step = VaultCreationStep::SetTimeDelay;
    assert_eq!(state.next_step_for_role(), Some(VaultCreationStep::MnemonicGeneration));
    ...
    state.device_role = DeviceRole::ViewOnly;
    state.current_step = VaultCreationStep::NameVault;
    assert_eq!(state.next_step_for_role(), Some(VaultCreationStep::ScanDescriptorViewOnly));
    
    // Test restore flow
    state.device_role = DeviceRole::Restore;
    state.current_step = VaultCreationStep::NameVault;
    assert_eq!(state.next_step_for_role(), Some(VaultCreationStep::EnterSeedPhrase));
}
...
fn test_vault_creation_step_just_changed() {
    ...
    assert!(changed, "step_just_changed should return true when step has changed");
    ...
    assert!(!state.step_just_changed(VaultCreationStep::NameVault), 
    ...
    assert!(state.step_just_changed(VaultCreationStep::SetTimeDelay),
