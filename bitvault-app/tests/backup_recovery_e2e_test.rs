...
use bitvault_app::services::key_service::{KeyService, BackupInfo};
...
async fn test_backup_seed_phrase_validation() {
    ...
    assert!(invalid_result.is_err(), "Invalid seed phrase should fail to parse");
}
...
async fn test_backup_key_derivation_from_seed() {
    ...
    assert!(!owner_keys.mainnet.owner_key1.is_empty(), "Mainnet keys should be derived");
    assert!(!owner_keys.testnet.owner_key1.is_empty(), "Testnet keys should be derived");
    
    // Verify same mnemonic produces same keys
    let owner_keys2 = get_owner_keys(&mnemonic).unwrap();
    assert_eq!(owner_keys.mainnet.owner_key1, owner_keys2.mainnet.owner_key1);
    assert_eq!(owner_keys.testnet.owner_key1, owner_keys2.testnet.owner_key1);
}
...
async fn test_backup_verification_flow() {
    ...
    assert_eq!(owner_keys.mainnet.owner_key1, verification_keys.mainnet.owner_key1);
    assert_eq!(owner_keys.testnet.owner_key1, verification_keys.testnet.owner_key1);
...
async fn test_restore_descriptor_validation() {
    ...
    assert!(!valid_descriptor.is_empty(), "Descriptor should not be empty");
    
    // Invalid descriptor (empty)
    let invalid_descriptor = "";
    assert!(invalid_descriptor.is_empty(), "Empty descriptor should be invalid");
    
    // Descriptor should contain key information
    assert!(valid_descriptor.contains("multi"), "Descriptor should contain multisig info");
}
...
async fn test_restore_flow_step_sequence() {
    ...
    use bitvault_app::ui::vault_creation::{VaultCreationStep, DeviceRole};
...
async fn test_restore_mnemonic_parsing() {
    ...
    assert!(parsed_trimmed.is_ok(), "Mnemonic with whitespace should parse after trim");
}
...
async fn test_restore_error_handling() {
    ...
    assert!(parse_result.is_err(), "Invalid mnemonic should produce error");
    
    // Empty descriptor should produce error
    let empty_descriptor = "";
    assert!(empty_descriptor.trim().is_empty(), "Empty descriptor should be invalid");
    
    // Missing vault name should produce error
    let empty_name = "";
    assert!(empty_name.trim().is_empty(), "Empty vault name should be invalid");
}
