...
fn test_pin_setup_flow_complete() {
    ...
    assert_eq!(validation_result.unwrap(), true, "Correct PIN should validate");
...
fn test_pin_setup_short_pin() {
    ...
    assert!(result.is_err(), "PinService should reject PINs that are not 6 digits");
    ...
    if should_skip_keyring_tests() {
        ...
        assert!(!pin_service.has_pin(), "PIN should not be set if format is invalid");
    }
...
fn test_pin_authentication_flow_incorrect_pin() {
    ...
    assert_eq!(result.unwrap(), false, "Incorrect PIN should validate to false");
    
    // Verify correct PIN still works
    let correct_result = pin_service.validate_pin(correct_pin);
    assert!(correct_result.is_ok());
    assert_eq!(correct_result.unwrap(), true, "Correct PIN should still work");
...
fn test_pin_service_delete_pin() {
    ...
    assert!(!pin_service.has_pin(), "PIN should not exist after deletion");
