...
async fn test_transaction_preview_validation() {
    ...
    assert!(!preview.recipient.is_empty(), "Recipient address should not be empty");
}
...
async fn test_transaction_amount_validation() {
    ...
    assert!(0.0 == 0.0, "Zero amount should be invalid (unless sending max)");
}
...
async fn test_transaction_address_validation() {
    ...
    assert!(Address::from_str(valid_testnet).is_ok(), "Valid testnet address should parse");
    
    // Invalid address
    let invalid = "invalid_address";
    assert!(Address::from_str(invalid).is_err(), "Invalid address should fail to parse");
    
    // Empty address
    let empty = "";
    assert!(Address::from_str(empty).is_err(), "Empty address should fail to parse");
}
...
async fn test_transaction_fee_calculation() {
    ...
    assert!(estimated_fee_btc < 0.001, "Fee should be reasonable for typical transaction");
}
...
async fn test_transaction_send_max_logic() {
    ...
    assert!(send_max_amount < balance_btc, "Send max amount should be less than balance");
    assert_eq!(send_max_amount + fee_btc, balance_btc, "Amount + fee should equal balance");
}
...
async fn test_transaction_preview_with_description() {
    ...
    assert!(preview.description.is_some(), "Description should be present");
...
async fn test_transaction_preview_without_description() {
    ...
    assert!(preview.description.is_none(), "Description should be optional");
}
...
async fn test_transaction_fee_rate_validation() {
    ...
    assert!(typical_fee_rate >= min_fee_rate, "Typical fee rate should be valid");
    
    // High fee rate (100+ sat/vB)
    let high_fee_rate = 100u64;
    assert!(high_fee_rate >= min_fee_rate, "High fee rate should be valid");
    
    // Zero fee rate should be invalid
    let zero_fee_rate = 0u64;
    assert!(zero_fee_rate < min_fee_rate, "Zero fee rate should be invalid");
}
...
async fn test_transaction_broadcast_validation() {
    ...
    assert!(!(not_signed && is_valid && has_sufficient_fee), 
