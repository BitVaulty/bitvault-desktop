...
use bitvault_app::ui::send_transaction::{SendTransactionState, HardwareWalletSigningMode};
...
fn test_send_transaction_hardware_wallet_flow() {
    ...
    assert_eq!(state.hw_signing_mode, HardwareWalletSigningMode::DisplayingQR);
...
fn test_send_transaction_success_handling() {
    ...
    assert!(state.success_message.as_ref().unwrap().contains("successfully"));
}
