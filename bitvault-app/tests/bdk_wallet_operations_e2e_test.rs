...
async fn test_wallet_initialization_with_descriptor() {
    ...
    assert!(
        service.is_loaded(),
        "Wallet should be loaded after initialization"
    );
    ...
    assert!(
        !new_address_str.is_empty(),
        "New address should not be empty"
    );
...
async fn test_wallet_transaction_building() {
    ...
    match preview_result {
        ...
        Err(e) => {
            ...
            println!(
                "Transaction building failed (expected for new wallet): {}",
                e
            );
        }
    }
...
async fn test_wallet_address_generation() {
    ...
    assert!(
        address2_result.is_ok(),
        "get_new_address() should succeed the second time"
    );
...
async fn test_wallet_transaction_history() {
    ...
    assert!(
        ...
        "Transaction list should be empty for new wallet"
    );
}
        ...
        Ok(preview) => {
            ...
            match sign_result {
                Ok(signed_psbt) => {
                    ...
                    assert_ne!(
                        signed_psbt, preview.psbt,
                        "Signed PSBT should differ from original"
                    );
                }
                Err(e) => {
                    ...
                    // We're testing BDK operations, not wallet funding
                    println!(
                        "PSBT signing failed (expected for new wallet or invalid PSBT): {}",
                        e
                    );
                }
            }
