...
async fn test_vault_creation_seed_phrase_verification() {
    ...
    assert!(
        wrong_result.is_err() || wrong_result.unwrap().to_string() != original_phrase,
...
async fn test_vault_creation_owner_keys_derivation() {
    ...
    assert!(
        ...
        "Mainnet owner_key1 should be derived"
    );
    assert!(
        !owner_keys.testnet.owner_key1.is_empty(),
        "Testnet owner_key1 should be derived"
    );
    ...
    assert!(
        !mainnet_key.is_empty(),
        "Mainnet key should not be empty, got: '{}'",
        mainnet_key
    );
    assert!(
        !testnet_key.is_empty(),
        "Testnet key should not be empty, got: '{}'",
        testnet_key
    );
...
async fn test_vault_creation_time_delay_validation() {
    ...
    let zero_delay = TimeDelay { days: 0, hours: 0 };
