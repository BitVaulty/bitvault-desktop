...
impl AsyncCommandHandler {
    ...
    pub fn process_pending(&mut self, vault_service: &std::sync::Arc<tokio::sync::RwLock<bitvault_common::wallet::VaultService>>, runtime: &tokio::runtime::Runtime) {
            ...
            match cmd {
                AsyncCommand::FetchBalance => {
                    ...
                    match result {
                        Ok((confirmed, available)) => {
                            let _ = tx.send(AsyncResult::Balance { confirmed, available });
                        }
                        Err(e) => {
                            let _ = tx.send(AsyncResult::Error(format!("Failed to fetch balance: {}", e)));
                        }
                    }
                }
                AsyncCommand::FetchAddress => {
                    ...
                    match result {
                        ...
                        Err(e) => {
                            let _ = tx.send(AsyncResult::Error(format!("Failed to fetch address: {}", e)));
                        }
                    }
                }
            }
