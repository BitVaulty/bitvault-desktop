        ...
        if app_state.is_vault_loaded() {
            ...
            if let Some(current_metadata) = app_state.get_current_vault_metadata() {
                ui.horizontal(|ui| {
                    ...
                    ui.label(format!("{} ({})", current_metadata.name, current_metadata.network));
                });
                ui.add_space(5.0);
            }
            ...
            match AppState::list_vaults() {
                Ok(all_vaults) => {
                    ...
                    let network_vaults: Vec<_> = all_vaults.iter()
                        .filter(|v| v.network == match app_state.network {
                            bdk::bitcoin::Network::Bitcoin => "mainnet",
                            bdk::bitcoin::Network::Testnet => "testnet",
                            bdk::bitcoin::Network::Signet => "signet",
                            bdk::bitcoin::Network::Regtest => "regtest",
                    ...
                    if network_vaults.len() > 1 {
                        ...
                        egui::ComboBox::from_id_source("vault_switcher")
                            .selected_text(
                                current_address.as_ref()
                                    .and_then(|addr| network_vaults.iter().find(|v| v.address == *addr))
                                    .map(|v| format!("{} ({})", v.name, &v.address[0..std::cmp::min(16, v.address.len())]))
                                    .unwrap_or_else(|| "Select vault".to_string())
                            )
                            .show_ui(ui, |ui| {
                                for vault in &network_vaults {
                                    let is_current = current_address.as_ref()
                                        .map(|addr| vault.address == *addr)
                                        .unwrap_or(false);
                                    
                                    let label = if is_current {
                                        format!("✓ {} ({})", vault.name, &vault.address[0..std::cmp::min(16, vault.address.len())])
                                    } else {
                                        format!("{} ({})", vault.name, &vault.address[0..std::cmp::min(16, vault.address.len())])
                                    };
                                    ...
                                    if ui.selectable_label(is_current, label).clicked() && !is_current {
                                        ...
                                        if let Some(ref runtime) = app_state.runtime {
                                            ...
                                            match handle.block_on(app_state.load_vault_from_metadata(&metadata)) {
                                                ...
                                                Err(e) => {
                                                    ui.colored_label(egui::Color32::RED, format!("Failed to load vault: {}", e));
                                                }
                                            }
                                        }
                                    }
                        ...
                        ui.label(format!("Only one vault available on {} network", app_state.network));
                    }
                }
                Err(e) => {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Export Vault").clicked() {
                    if let Some(ref address) = export_address {
                        match AppState::list_vaults() {
                            Ok(vaults) => {
                                if let Some(metadata) = vaults.iter().find(|v| v.address == *address) {
